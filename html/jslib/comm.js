...
**/
voyc.Comm = function(base, name, retries, consolidated) {
	this.base = base || window.location.origin;
	this.name = name || '';  // name of this object, for debug tracing
	this.seq = 1;
...
voyc.Comm.prototype = {
	/**
		...
		lang - svc or pvc, default to svc
	**/
	request: function(svc, data, callback, lang) {
...
voyc.Xhr = function() {
	...
	this.callback = null;
	this.lang = 'svc';   // svc:php, pvc:python
	this.retries = 0;
	this.maxretries = 3;
	this.seq = 0;
	this.start = 0;
...
voyc.Xhr.prototype = {
	callServer :function() {
		...
		var url = this.composeUrl(this.base, this.lang, this.svc);
		var data = this.data;
	...
	// compose url from components
	composeUrl: function(base, lang, svc) { 
		url = base + '/' + lang + '/' + svc
		return url
	}
}
